function TwoGatherAPI() {
	...
	this.checkTx = function(txHash){
		...
		// failed
		else if (txObj.status === 2){
			delete txData[txHash];
			return 2;
		} else if (txObj.status === 3){
			// Tx was processed, but don't return 3, first run the test to see if the
			// update it was expected to do actually happened.
			delete txData[txHash];
			return 3;
		} else {
			// Pending
	...
	this.statUpdatePre = function(event){
	...
	function vidObjToVidData(vidObj,channelAddr){
		Printf("VIDOBJ: %v\n",vidObj);
		var vdat = {};
		vdat.name = sutil.hexToString(esl.kv.Value(channelAddr, StringToHex("vidnames"), vidObj.Key));
	...
	*/
	...
	this.sub = function() {
		events.subscribe("monk","newTx:pre","", this.statUpdatePre, "does_not_matter");
		events.subscribe("monk","newTx:post","", this.statUpdatePost, "does_not_matter");
		events.subscribe("monk","newTx:pre:fail","", this.statUpdateFail, "does_not_matter");
		events.subscribe("monk","newTx:post:fail","", this.statUpdateFail, "does_not_matter");
	}

	// No websockets running here, meaning there's only one possible sub from this runtime.
	function unsub(){
		events.unsubscribe("monk","newTx:pre","does_not_matter");
		events.unsubscribe("monk","newTx:post","does_not_matter");
		events.unsubscribe("monk","newTx:pre:fail","does_not_matter");
		events.unsubscribe("monk","newTx:post:fail","does_not_matter");
	}
